...
def MCpi(a,n):
    ...
    for i in range(n):
        ...
        if (distanceFromO <= rSquared): n_in_circle += 1
        n_in_square += 1
    try:
...
def isGateOutputHigh(gateName, vector):
    if   gateName == "AND":     return (0 not in vector)
    elif gateName == "NAND":    return (0 in vector)
    elif gateName == "OR":      return (1 in vector)
    elif gateName == "NOR":     return (1 not in vector)
    elif gateName == "XOR":
        result = False
        for input in vector:
...
def MCGate(gateName, NumOfInputs, n, doSp=0, spList=[]):
    ...
    for vector in workload:
        new_gate_output = 0
        if isGateOutputHigh(gateName, vector):      new_gate_output = 1
