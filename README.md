...
## To run the project
  ...
  3. For **Lab5**, open the .v files with your HDL simulator, compile them, and start a simulation for one of the testbench files that can be found in
     **/Lab5/src**. If you are using *Modelsim* or *QuestaSim*, you can load the *.do* files found inside **Lab5/sim** by selecting **File > Load > Macro File**        and choosing one of *.do* files, or by typing ```do macro_file_name.do``` inside the Transcript console. Run the simulation by typing the command
     ```run -all``` in the Transcript window or by pressing the *Run -All* button.
